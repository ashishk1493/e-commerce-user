...
import { deleteCartProductById } from 'services/products.service';
...
export default function CheckoutProductList({ products, onDelete, onIncreaseQuantity, onDecreaseQuantity }) {
  ...
  const deleteCartProduct = async (product_id) => {
    ...
    const response = await deleteCartProductById(product_id)
      ...
      <Table>
        ...
        <TableBody>
          {products?.map((product) => {
            ...
            const { color, size, productDetail, id, price, qty, total_price, product_id } = product;
            return (
              <TableRow key={id}>
                <TableCell>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <Image alt="product image" src={'http://localhost:8080' + productDetail.images[0]} sx={{ width: 64, height: 64, borderRadius: 1.5, mr: 2 }} />
                    <Box>
                ...
                <TableCell align="left">
                  <Incrementer
                    quantity={qty}
                    available={productDetail.availableQty}
                    onDecrease={() => {
                      // notifyError("Error avi gay");
                      onDecreaseQuantity(product_id)
                    }}
                    onIncrease={() => {
                      // notifySuccess("Maja aa gaya");
                      onIncreaseQuantity(product_id)
                    }}
                  />
                </TableCell>
                ...
                <TableCell align="right">
                  <IconButton onClick={() => deleteCartProduct(product_id)}>
