...
export default function ProductDetailsCarousel({ product }) {
  ...
  const imagesLightbox = product.images.map((_image) => 'http://localhost:8080' + _image);
            ...
            {product.images.map((img) => {
              ...
              return (
                <Image
                  ...
                  ratio="1/1"
                  onClick={() => handleOpenLightbox('http://localhost:8080' + img)}
                  sx={{ cursor: 'zoom-in', objectFit: "contain !important" }}
                />
              )
            })}
          </Slider>
